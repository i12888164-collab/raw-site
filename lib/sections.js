...
export const SECTIONS = {
  ...
  "sportswear": {
    ...
    bg: "#121c28",
    surface: "#1a2633",
    heroImage: "/banners/sportswear.jpg",
...
// Акцентный цвет раздела (для подсветки в админке)
export function sectionAccent(slug) {
  return SECTIONS[slug]?.accent || "#c9a96e";
}

export function orderLinks(product, pkg) {
  let label = product.category && product.category !== product.name
    ? `${product.name} — ${product.category}`
    : product.name;
  if (pkg) {
    label += ` [${pkg.amount.toLocaleString("ru-RU")} ${pkg.currency}]`;
  } else if (product.variant) {
    label += ` (${product.variant})`;
  }
  const text = encodeURIComponent(`Здравствуйте! Хочу заказать: ${label}`);
  return {
